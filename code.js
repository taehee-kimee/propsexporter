...
async function ExportComponentProperties(options) {
    ...
    try {
        ...
        if (node.type === 'COMPONENT') {
            ...
            const result = await withTimeout(ExportFromComponent(node, options), 30000, 'Export operation timed out after 30 seconds');
            ...
            const result = await withTimeout(ExportFromComponentSet(node, options), 30000, 'Export operation timed out after 30 seconds');
 ...
 */
function ExportProperties(componentProperties) {
    const props = {};
...
function ExportAnatomy(node) {
    ...
    if (node.type === 'COMPONENT_SET' && node.children.length > 0) {
        const variantAnatomies = {};
        for (const variant of node.children) {
            if (variant.type === 'COMPONENT') {
                const variantName = variant.name;
                const anatomy = [];
                function traverseNode(n, path = '', parentPath = '') {
...
async function ExportFigmaStyles(node) {
    ...
    async function collectStyles(n) {
        try {
            ...
            if ('fillStyleId' in n) {
                ...
                const fillStyleId = n.fillStyleId;
            ...
            if ('strokeStyleId' in n) {
                const strokeStyleId = n.strokeStyleId;
            ...
            if (n.type === 'TEXT') {
                ...
                if (textStyleId !== figma.mixed && typeof textStyleId === 'string' && textStyleId !== '') {
                    try {
                        ...
                        console.log('[collectStyles] Got text style:', (style === null || style === void 0 ? void 0 : style.name) || 'null');
                        if (style) {
            ...
            if ('effectStyleId' in n) {
                ...
                const effectStyleId = n.effectStyleId;
    ...
    // Remove empty arrays
    const result = {};
    if (figmaStyles.fillStyles.length > 0)
        result.fillStyles = figmaStyles.fillStyles;
    if (figmaStyles.strokeStyles.length > 0)
        result.strokeStyles = figmaStyles.strokeStyles;
    if (figmaStyles.textStyles.length > 0)
...
async function ExportTokens(node) {
            ...
            if (boundVariables) {
                    ...
                    for (const fillVar of boundVariables.fills) {
                        if (fillVar && 'id' in fillVar) {
                            try {
                                ...
                                const variable = await withTimeout(figma.variables.getVariableByIdAsync(fillVar.id), 5000, 'Variable fetch timeout');
                ...
                for (const mapping of propertyMappings) {
                    if (mapping.prop in boundVariables) {
                        ...
                        if (varRef && 'id' in varRef) {
                            try {
                                const variable = await withTimeout(figma.variables.getVariableByIdAsync(varRef.id), 5000, 'Variable fetch timeout');
                                if (variable) {
    ...
    if (tokens.sizing.length > 0)
        result.sizing = tokens.sizing;
 ...
 */
async function getComponentsHash() {
    const componentIds = [];
    function findComponentIds(node) {
...
async function ExportMultipleComponents(componentIds, options) {
    try {
        ...
        for (const id of componentIds) {
            ...
            if (node.type === 'COMPONENT') {
                const result = await withTimeout(ExportFromComponent(node, options), 30000, `Export timeout for component: ${node.name}`);
                Object.assign(results, result);
            }
            else if (node.type === 'COMPONENT_SET') {
                const result = await withTimeout(ExportFromComponentSet(node, options), 30000, `Export timeout for component set: ${node.name}`);
                Object.assign(results, result);
            }
        }
